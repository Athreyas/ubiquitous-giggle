...
| LOG_LEVEL                              | No                                    | debug           | The application log level as defined in the [winston documentation](https://github.com/winstonjs/winston?tab=readme-ov-file#logging-levels). You may want to set this to `notice` or `warning` when running Daykeep in a production environment.                      |
...
Daykeep supports two storage backends for assets: local filesystem (default) and S3-compatible object storage. S3 storage is automatically detected when an S3 endpoint is passed.
...
By default, Daykeep uses the database to store users, but it is possible to also use OAuth.
...
:::info
When setting up OAuth, the allowed redirect URLs configured at the provider should be set to `<KARAKEEP_ADDRESS>/api/auth/callback/custom` where `<KARAKEEP_ADDRESS>` is the address you configured in `NEXTAUTH_URL` (for example: `https://try.daykeep.app/api/auth/callback/custom`).
:::
...
Daykeep can send emails for various purposes such as email verification during signup. Configure these settings to enable email functionality.
...
If your Daykeep instance needs to connect through a proxy server, you can configure the following settings:
...
Daykeep supports distributed tracing and structured event logging via OpenTelemetry. When enabled, traces and logs are collected for tRPC API calls, background worker operations, and other key workflows. Daykeep also exports prometheus-based metrics.
...
| OTEL_EXPORTER_OTLP_TRACES_ENDPOINT | No       | Not set  | The OTLP HTTP endpoint to send traces to (e.g., `http://jaeger:4318/v1/traces` or `http://otel-collector:4318/v1/traces`). If not set, traces are logged to the console.                                                                           
...
| EVENT_LOGS_ENABLED                 | No       | false    | Set to `true` to enable structured event logging. Event logs log various interesting events that happens through the system (e.g. login, bookmark creation, etc). Those can be very chatty. When enabled, they'll get logged to the console unless OTEL logs exporter is enabled.                                                                                                    |
