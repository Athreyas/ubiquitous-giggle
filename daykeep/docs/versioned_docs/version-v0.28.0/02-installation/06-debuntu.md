...
The script will download and install all dependencies (except for Ollama), install Daykeep, do a basic configuration of Daykeep and Meilisearch (the search app used by Daykeep), and create and enable the systemd service files needed to run Daykeep on startup. Daykeep and Meilisearch are run in the context of their low-privilege user environments for more security.

The script functions as an update script in addition to an installer. See **[Updating](#updating)**.

### 1. Download the script from the [Daykeep repository](https://github.com/karakeep-app/karakeep/blob/main/karakeep-linux.sh)
...
> This script must be run as `root`, or as a user with `sudo` privileges.
    ...
    If Daykeep has previously been installed using this script, then run the updater like so:
...
- `meilisearch.service`: Provides full-text search, Daykeep Workers service connects to it, uses port `7700` by default.
...
During installation, the script created a configuration file for `meilisearch`, an `ENV` file for Daykeep, and located config paths and database paths separate from the installation path of Daykeep, so as to allow for easier updating. Their names/locations are as follows:
...
- `/etc/karakeep/karakeep.env` - The Daykeep `ENV` file. Edit this file to configure Daykeep beyond the default. The web service and the workers service need to be restarted after editing this file:

    ```shell
    sudo systemctl restart karakeep-workers karakeep-web
    ```

- `/var/lib/karakeep` - The Daykeep database location. If you delete the contents of this folder you will lose all your data.

## Still Running Daykeep?

There is a way to upgrade. Please see [Guides > Daykeep to Daykeep Migration](https://docs.daykeep.app/Guides/hoarder-to-karakeep-migration)
